...
# Edition: Fri 12 Sep 2025 20:50:19 BST
###############################################################################
...
if is_macos; then
    # Preload keys to the ssh agent from keychain
    ssh-add --apple-use-keychain ~/.ssh/id_rsa 2>/dev/null
elif is_ubuntu; then
    # Ubuntu: Use keychain or ssh-agent
...
load_zsh_plugins() {
    ...
    if command -v brew &>/dev/null; then
        ...
        # MacPorts detected - confirmed paths
...
if (( $+functions[compdef] )); then
    _go_completion() {
        local bookmarks=($(grep "^alias go_" "$BOOKMARKS_FILE" 2>/dev/null | \
            sed 's/alias go_//;s/=.*//' || true))
...
export BOBLBEE_DIR="$HOME/Developer/workspace/matdotcx/boblbee"

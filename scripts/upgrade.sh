...
#
# The entire script is wrapped in main() so bash parses it
# fully before execution. This is important because the
# git pull inside the script replaces this file on disk —
# without the wrapper, bash could read stale/shifted lines
# after the pull overwrites the file mid-execution.
#########################################################
...
main() {
    ...
    if ! git diff --quiet || ! git diff --staged --quiet; then
        ...
        echo -e "${YELLOW}Local changes stashed${NC}"
    fi
    ...
    if [ -L "$HOME/.config/claude/memory/user.md" ]; then
        echo -e "${YELLOW}Claude memory is a symlink (will be migrated to copy)${NC}"
    elif [ -f "$HOME/.config/claude/memory/user.md" ]; then
        echo -e "${GREEN}✓ Claude memory exists${NC}"
    else
